...
# Initialize loggers
fetch_logger_dir = "Logs/Comments"
...
def fetch_comments(ticket_id, retry_count=0):
    """Fetch comments from Zendesk for a given ticket ID."""
    ...
    try:
        ...
        if response.status_code == 429:
            ...
            time.sleep(wait_time)
            return fetch_comments(ticket_id, retry_count + 1)
        ...
        count = data.get('count', len(comments))  # Extract count (default to len if missing)
...
def save_comments_chunked(ticket_comments, batch_number):
    """Save 10,000 records per file."""
    filename = f"comments_{batch_number}.json"
    try:
        with open(filename, 'w', encoding='utf-8') as output_file:
            json.dump(ticket_comments, output_file, indent=4)
        console_logger.info(f"Saved {len(ticket_comments)} records to {filename}")
    except IOError as e:
        error_logger.error(f"Error saving to {filename}: {e}")

def process_tickets():
    """Process tickets by fetching comments and saving data in chunks of 10,000."""
    ticket_comments = {}
    batch_number = 1
    processed_count = 0
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(fetch_comments, ticket_id): ticket_id for ticket_id in ticket_ids}

        for future in concurrent.futures.as_completed(futures):
            ticket_id = futures[future]
            try:
                comments, count = future.result()
                ...
                if processed_count % BATCH_SIZE == 0:
                    save_comments_chunked(ticket_comments, batch_number)
                    batch_number += 1
                    ticket_comments.clear()  # Clear memory after saving
